...
SSL *engine::native_handle() { return ssl_; }

asio::error_code engine::set_verify_mode(verify_mode v, asio::error_code &ec) {
...
asio::error_code engine::set_verify_depth(int depth, asio::error_code &ec) {
...
asio::error_code engine::set_verify_callback(verify_callback_base *callback,
                                             asio::error_code &ec) {
  if (SSL_get_app_data(ssl_))
    delete static_cast<verify_callback_base *>(SSL_get_app_data(ssl_));
...
int engine::verify_callback_function(int preverified, X509_STORE_CTX *ctx) {
  if (ctx) {
    if (SSL *ssl = static_cast<SSL *>(::X509_STORE_CTX_get_ex_data(
            ctx, ::SSL_get_ex_data_X509_STORE_CTX_idx()))) {
      if (SSL_get_app_data(ssl)) {
        verify_callback_base *callback =
            static_cast<verify_callback_base *>(SSL_get_app_data(ssl));
...
engine::want engine::handshake(stream_base::handshake_type type,
                               asio::error_code &ec) {
...
engine::want engine::write(const asio::const_buffer &data, asio::error_code &ec,
  ...
  return perform(&engine::do_write, const_cast<void *>(data.data()),
...
engine::want engine::read(const asio::mutable_buffer &data,
  ...
  return perform(&engine::do_read, data.data(), data.size(), ec,
                 &bytes_transferred);
}

asio::mutable_buffers_1 engine::get_output(const asio::mutable_buffer &data) {
...
asio::const_buffer engine::put_input(const asio::const_buffer &data) {
  int length =
      ::BIO_write(ext_bio_, data.data(), static_cast<int>(data.size()));
...
engine::want engine::perform(int (engine::*op)(void *, std::size_t), void *data,
                             std::size_t length, asio::error_code &ec,
                             std::size_t *bytes_transferred) {
...
int engine::do_connect(void *, std::size_t) { return ::SSL_connect(ssl_); }
